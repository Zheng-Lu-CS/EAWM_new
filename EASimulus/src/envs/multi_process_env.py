...
def child_env(child_id: int, env_fn: Callable, child_conn: Connection, parent_conns: List[Connection]) -> None:
...
class MultiProcessEnv(DoneTrackerEnv):
    def __init__(self, env_fn: Callable, num_envs: int, should_wait_num_envs_ratio: float) -> None:
        ...
        for child_id in range(num_envs):
            ...
            p = Process(target=child_env, args=(child_id, env_fn, child_conn, self.parent_conns), daemon=True)
            self.processes.append(p)
        for p, child_conn in zip(self.processes, child_conns):
            p.start()
            child_conn.close()
    ...
    def close(self) -> None:
        if self.closed:
            return
        self.closed = True
        for parent_conn, process in zip(self.parent_conns, self.processes):
            if process.is_alive():
